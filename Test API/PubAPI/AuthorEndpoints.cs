...
public static class AuthorEndpoints
{
    ...
    public static void MapAuthorEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/datalogic/{authorid}", async Task<Results<Ok<AuthorDTO>, NotFound>> (int authorid, PubContext db) =>
        {
            var dl=new DataLogic(db);
            return await dl.GetAuthorById(authorid)
                is AuthorDTO model
                    ? TypedResults.Ok(model)
                    : TypedResults.NotFound();
        })
