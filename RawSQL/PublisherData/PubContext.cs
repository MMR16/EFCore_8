...
public class PubContext : DbContext
{
    ...
    public DbSet<AuthorByArtist> AuthorsByArtist { get; set; } // the same name as view name // keyless entity
    ...
    {
        modelBuilder.Entity<AuthorByArtist>().HasNoKey().ToView(nameof(AuthorsByArtist)); //view  keyless entity (has no primary key)
        ...
        //InsertUsingStoredProcedure to insert using exist stored procedure
        modelBuilder.Entity<Author>()
            .InsertUsingStoredProcedure("authorInsert", e =>
                e.HasParameter(w => w.FirstName)
                 .HasParameter(w => w.LastName)
                 .HasResultColumn(w => w.AuthorId)
            );
    }
}