...
public class PubContext : DbContext
{
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        optionsBuilder.UseSqlServer( 
          "Data Source = (localdb)\\MSSQLLocalDB; Initial Catalog = PubsDatabase"
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Artist>()
            .HasMany(e => e.Covers)
            .WithMany(q => q.Artists)
            .UsingEntity<CoverAssignment>(e =>
            {
                e.Property(q => q.DateCreated).HasDefaultValueSql("CURRENT_TIMESTAMP"); // For SQL Server
                e.Property(q => q.CurrentUser).HasDefaultValueSql("CURRENT_USER"); // For SQL Server 
                e.ToTable("ArtistCover");
                e.Property(q => q.CoverId).HasColumnName("CoversCoverId");
                e.Property(q => q.CoverId).HasColumnName("ArtistArtistId");
            });
        ...
        //   .HasMany(a => a.Books)
        //   .WithOne(b => b.Author)
        //   .HasForeignKey("AuthorFK").IsRequired(false);
